...
<div class="section-header">
    ...
    <div class="section-header-breadcrumb">
        <div class="breadcrumb-item active"><a href="#">Invoice Toko</a></div>
    </div>
</div>

<div class="section-body">
    <a class="btn btn-icon icon-left btn-primary float-right" href="{{ url('invoice/create') }}"><i class="fas fa-plus"></i>
        Invoice</a>
    <h2 class="section-title">Invoice</h2>
    <p class="section-lead">
        Berikut adalah data invoice toko melindastore</a>.
    </p>
    <div class="row">
        <div class="col-12">
            <div class="card">
...
$(document).ready(function() {
    ...
    $(document).on('click','.btn-convert', function(e) {
        ...
        data.id = $(this).data('id');
        Swal.fire({
          title: 'Are you sure to convert invoice to penjualan?',
          text: "You won't be able to revert this!",
          icon: 'warning',
          showCancelButton: true,
          confirmButtonColor: '#3085d6',
          ...
          if (result.value) {
            window.location = '/penjualan/convert-invoice/'+data.id;
          }
      })
    });
    ...
    $('.filter').on('click', function(e) {
        ...
        if($('.invalid-feedback:visible').length == 0){
            ...
            $('#datatable-new').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ url('invoice/show') }}"+'?date_from='+date_from+'&date_to='+date_to+'&toko='+toko,
                columns: [
                    { data: 'id', name: 'id' },
                    { data: 'tanggal', name: 'tanggal', orderData: 2},
                    { data: 'date', name: 'date', visible: false},
