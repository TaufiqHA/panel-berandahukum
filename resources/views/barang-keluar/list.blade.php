...
<div class="section-body">
    <a class="btn btn-icon icon-left btn-primary float-right" href="{{ url('barang-keluar/create') }}"><i class="fas fa-plus"></i>
        Barang Keluar</a>
    <h2 class="section-title">Barang Keluar</h2>
    <p class="section-lead">
        Berikut adalah data barang keluar melindastore</a>.
    </p>
    <div class="row">
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form class="needs-validation" novalidate="">
                        <div class="form-row">
                          <div class="form-group {{ $user->status == 1 ? "col-md-3" : "col-md-6" }}">
                          ...
                          @endif
                          <div class="form-group {{ $user->status == 1 ? "col-md-2" : "col-md-6" }}">
...
$(document).ready(function() {
    ...
    var table = $('#datatable').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ url('barang-keluar/show') }}",
        columns: [
            { data: 'id', name: 'id' },
			{ data: 'tanggal', name: 'tanggal', orderData: 2},
            { data: 'date', name: 'date', visible:false},
            { data: 'kode_barang_keluar', name: 'kode_barang_keluar' },
    ...
    $('.filter').on('click', function(e) {
        ...
        if($('.invalid-feedback:visible').length == 0){
            ...
            $('#datatable-new').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ url('barang-keluar/show') }}"+'?date_from='+date_from+'&date_to='+date_to+'&toko='+toko,
                columns: [
                    { data: 'id', name: 'id' },
